...
public class BuildingRepository extends SimpleRepository<BuildingEntity>{
	...
	public List<BuildingEntity> findAll(Map<String,Object> properties,
		...
		results.forEach(e->{
			...
			e.setRentArea(rentArea.toArray(new RentArea[rentArea.size()]));
		});
		return results;
	...
	public BuildingEntity findById(int id) {
		BuildingEntity entity = super.findById((long)id);
		RentAreaRepository rentAreaRepository = new RentAreaRepository();
		List<RentArea> rentArea = rentAreaRepository.findByBuildingId(entity.getId().intValue());
		entity.setRentArea(rentArea.toArray(new RentArea[rentArea.size()]));
		return entity;
	}
